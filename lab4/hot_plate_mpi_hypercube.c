...
int allbcast(int done, int numdim)
{
	int rank;
    ...
    for(curdim = 0; curdim < numdim; curdim++) {
		if ((rank & notparticipating) == 0) {
			if ((rank & bitmask) == 0) {
				...
				MPI_Isend(&done, 1, MPI_INT, msg_dest, 0, MPI_COMM_WORLD, &request);
			} 
			else {
				int temp_done = 0;
				int msg_src = rank ^ bitmask;
				MPI_Recv(&temp_done, 1, MPI_INT, msg_src, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE); 
				if (temp_done < done) {
					done = temp_done;
				}
			}
...
int main(int argc, char *argv[])
{
    ...
    while(reallydone != 1)
    {
		...
		reallydone = allbcast(done, numdimensions);
