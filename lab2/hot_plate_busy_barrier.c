...
pthread_mutex_t busybarrier_lock;
...
void mylib_init_barrier(barrier_node* b, int nthreads)
{
	...
	pthread_mutex_init(&busybarrier_lock, NULL);
}
...
void busybarrier(barrier_node* b, int num_threads, int thread_id)
{
	if (num_threads == 1)
		return;

	pthread_mutex_lock(&busybarrier_lock);
	number_in_barrier++;
	if (number_in_barrier == num_threads) {

		pthread_mutex_unlock(&busybarrier_lock);
		number_in_barrier = 0;
	}
	else {
		pthread_mutex_unlock(&busybarrier_lock);
		while (number_in_barrier != 0) { }
	}
...
void* update_plate(void* plate_arguments) {
	...
	for (;;) {
		...
		busybarrier(barr, plate_args->nthreads, plate_args->iproc); 
		...
		busybarrier(barr, plate_args->nthreads, plate_args->iproc); //wait for all threads to finish updating here
